...
- (void)storeIntoDiskWithImage:(UIImage *)image recalculateFromImage:(BOOL)recalculate imageData:(NSData *)imageData forKey:(NSString *)key
{
    dispatch_async(self.ioQueue, ^{
        NSData *data = imageData;
...
- (void)storeImage:(UIImage *)image forKey:(NSString *)key toMemory:(BOOL)toMemory
{
    ...
    [self storeIntoDiskWithImage:image recalculateFromImage:YES imageData:nil forKey:key];
}
