...
const float voiceHoldButtonHeight = 45;
...
- (void)messageStyleButtonClicked:(UIButton *)sender {
    ...
    switch (index) {
        case kVoiceKeyBoardButtonTag: {
            ...
            [MobClick event:@"chat_start_recording"];
...
- (void)setupMessageInputViewBarWithStyle:(SnailMessageInputViewStyle)style {
    ...
    if (self.allowsSendVoice) {
        button = [self createButtonWithImage:[UIImage imageNamed:@"ico_chat_voice.png"] HLImage:[UIImage imageNamed:nil]];
        [button addTarget:self action:@selector(messageStyleButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
        button.tag = 0;
        [button setBackgroundImage:[UIImage imageNamed:@"ico_chat_keyboard.png"] forState:UIControlStateSelected];
    ...
    if (self.allowsSendMultiMedia) {
        button = [self createButtonWithImage:[UIImage imageNamed:@"ico_chat_more.png"] HLImage:[UIImage imageNamed:nil]];
    ...
    if (self.allowsSendFace) {
        button = [self createButtonWithImage:[UIImage imageNamed:@"ico_chat_face.png"] HLImage:[UIImage imageNamed:@"face_HL"]];
        button.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin;
        [button setBackgroundImage:[UIImage imageNamed:@"ico_chat_keyboard.png"] forState:UIControlStateSelected];
        [button addTarget:self action:@selector(messageStyleButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
        button.tag = 1;
    ...
    if (self.allowsSendVoice) {
        ...
        //增加录音按钮高度
        CGRect buttomFrame = CGRectMake(_inputTextView.frame.origin.x, _inputTextView.frame.origin.y - (voiceHoldButtonHeight - _inputTextView.frame.size.height)/2, _inputTextView.frame.size.width, voiceHoldButtonHeight);
        
        buttonFrame = buttomFrame;
        button.frame = buttonFrame;
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    if ([text isEqualToString:@""]) {
        if ([TQRichTextEmojiRun deleteLastEmojiStrForTextView:textView]) {
            return NO;
        }
    }
